...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const models = gatewayDb
    .prepare("SELECT id, alias, real_model, channel_id, is_public, enabled, weight, created_at FROM models WHERE channel_id = ? AND deleted_at IS NULL ORDER BY id DESC")
    .all(id);
