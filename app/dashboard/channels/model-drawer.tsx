              ...
              <div className="space-y-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-hover)] p-4">
                ...
                {form.quota_mode === "independent" ? (
                  <>
                    ...
                    <div className="border-t border-[var(--color-border)] pt-3">
                      <p className="mb-1 text-xs font-medium text-[var(--color-foreground-muted)]">每用户独立限额</p>
                      <p className="mb-3 text-xs text-[var(--color-foreground-muted)]">可选，为每个用户设置独立的模型配额，留空表示不限制。</p>
                      <div className="grid gap-3 md:grid-cols-2">
                        <div className="space-y-2">
                          <Label>每用户请求配额</Label>
                          <Input type="number" min={0} value={form.per_user_quota_requests} onChange={(e) => onFormChange({ per_user_quota_requests: e.target.value })} placeholder="留空不限制" />
                        </div>
                        <div className="space-y-2">
                          <Label>每用户 Token 配额</Label>
                          <Input type="number" min={0} value={form.per_user_quota_tokens} onChange={(e) => onFormChange({ per_user_quota_tokens: e.target.value })} placeholder="留空不限制" />
                        </div>
                      </div>
                      {periodQuotaEnabled ? (
                        <div className="mt-3 border-t border-[var(--color-border)] pt-3">
                          <p className="mb-3 text-xs font-medium text-[var(--color-foreground-muted)]">每用户周期配额</p>
                          <div className="grid gap-3 md:grid-cols-2">
                            <div className="space-y-2">
                              <Label>重置周期</Label>
                              <Select
                                value={form.per_user_quota_period_preset || "none"}
                                onValueChange={(value) => onFormChange({
                                  per_user_quota_period_preset: value === "none" ? "" : value,
                                  per_user_quota_period_custom: value === "custom" ? form.per_user_quota_period_custom : "",
                                })}
                              >
                                <SelectTrigger>
                                  <SelectValue placeholder="不限制" />
                            ...
                            {form.per_user_quota_period_preset === "custom" ? (
                              <div className="space-y-2">
                                <Label>自定义周期（秒）</Label>
                                <Input
                                  type="number"
                                  min={60}
                                  value={form.per_user_quota_period_custom}
                                  onChange={(e) => onFormChange({ per_user_quota_period_custom: e.target.value })}
                                  placeholder="如 7200 = 2小时"
                                />
                              </div>
                            ) : <div />}
                            <div className="space-y-2">
                              <Label>每用户周期请求配额</Label>
                              <Input
                                type="number"
                                min={0}
                                value={form.per_user_period_quota_requests}
                                onChange={(e) => onFormChange({ per_user_period_quota_requests: e.target.value })}
                                placeholder="留空不限制"
                              />
                            </div>
                            <div className="space-y-2">
                              <Label>每用户周期 Token 配额</Label>
                              <Input
                                type="number"
                                min={0}
                                value={form.per_user_period_quota_tokens}
                                onChange={(e) => onFormChange({ per_user_period_quota_tokens: e.target.value })}
                                placeholder="留空不限制"
                              />
                            </div>
                          </div>
