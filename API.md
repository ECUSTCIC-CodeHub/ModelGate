...
| 版本 | 取值 | 功能范围 |
|:---|:---|:---|
| 完整版 | `full`（默认） | 包含 OIDC、周期配额、系统公告等完整功能 |
| 精简版 | `lite` | 不包含 OIDC、周期配额、系统公告、Webhook 回调；相关页面不渲染，相关 API 功能不可用或忽略对应字段 |
...
- 系统公告接口返回 404，系统设置中的 `announcement_content` 不返回实际内容，更新时忽略。
...
> `prompt_tokens`、`completion_tokens`、`total_tokens` 为实际用于扣量和汇总的 Token 用量：优先采用远端 usage，远端缺失时回退本地统计。日志中的 `completion_tokens` 表示可见输出 Token，不包含单独识别出的思考 Token；`total_tokens` 仍包含思考消耗。`metadata.token_usage.remote` 记录上游返回的 usage 原始 Token 数，其中 `text_tokens` 表示远端明细里的可见文本 Token，`reasoning_tokens` 表示思考 Token，`cache` 记录上游返回的缓存命中/读取、缓存创建/写入、缓存未命中 Token。`metadata.token_usage.local` 记录本地分词统计结果。`output_tps` 使用可见输出 Token 计算，不把思考 Token 计入输出速度。`token_source` 表示本次采用来源：`usage` 为上游响应返回的 usage 字段，`local` 为本地 GPT 分词器兜底统计，`estimated` 为请求失败时的预估值。`user_agent` 记录客户端请求携带的 User-Agent，最长保留 500 字符。普通用户不会看到 `username`、`channel_name`、`route_attempts`、`attempted_channels` 字段。
...
> 周期配额响应头仅在配置了周期配额时返回；精简版不会返回 `X-Period-*` 响应头。
