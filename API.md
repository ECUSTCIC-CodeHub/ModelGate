  ...
  "data": [
    {
      ...
      "metadata": {
        "token_usage": {
          "remote": { "prompt_tokens": 100, "completion_tokens": 50, "total_tokens": 150 },
          "local": { "prompt_tokens": 98, "completion_tokens": 49, "total_tokens": 147 },
...
> `prompt_tokens`、`completion_tokens`、`total_tokens` 为实际用于扣量和汇总的 Token 用量：优先采用远端 usage，远端缺失时回退本地统计。`metadata.token_usage.remote` 记录上游返回的 usage 原始 Token 数，`metadata.token_usage.local` 记录本地分词统计结果；`metadata.token_usage.cache` 记录缓存命中/读取、缓存创建/写入、缓存未命中 Token。`token_source` 表示本次采用来源：`usage` 为上游响应返回的 usage 字段，`local` 为本地 GPT 分词器兜底统计，`estimated` 为请求失败时的预估值。`user_agent` 记录客户端请求携带的 User-Agent，最长保留 500 字符。普通用户不会看到 `username`、`channel_name`、`route_attempts`、`attempted_channels` 字段。
