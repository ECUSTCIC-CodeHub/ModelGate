...
邮件通知用于在创建/修改系统公告时，向「用户列表中填写了邮箱且处于启用状态」的用户发送通知邮件。支持配置多个 SMTP 发件账号，并按优先级与单日发送上限自动分流：发送时从优先级最高的账号开始、同优先级间轮流分配，达到单日上限的账号当日不再分配，全部账号额度用尽时剩余用户当日不再发送。
...
| footer | string | 邮件正文页脚，附加在公告内容之后 |
...
重发此前发送失败的公告邮件。仅针对 `email_send_log` 中状态为 `failed` 的记录，按公告分组补发，且**不占用发件账号的单日发送额度**。补发成功后会将该记录标记为 `sent`，再次失败仍保持 `failed` 可继续重发。
