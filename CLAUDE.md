...
- 新修改必须走 PR 流程：完成改动后创建独立分支推送到远端并提交 PR，禁止直接向 `main` 提交；PR 是代码审计与 CI/CD 流水线的载体，跳过 PR 直接提交会丢失流水线关联与变更记录
- 不主动合并 PR、不主动推送 `main`，等用户指示；为提 PR 推送分支属于流程必需
...
1. 通过 `git diff main...<PR 分支>` / `git status` 获取 PR 的变更范围（审计在 PR 分支上进行，不审计 `main` 上已合入的内容）
...
- 审计通过前不得合并 PR，也不得将改动直接提交到 `main`；审计通过并修复低危项后，将 PR 交给用户确认合并时机
