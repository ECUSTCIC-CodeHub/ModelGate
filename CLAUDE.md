...
- body 使用 `- ` 列出本次提交的具体事项
- 功能变更同时更新 `API.md`（中文）
- 回复用户前应对本次新产生的尚未提交的代码做审计（环境具备 `codebuddy` 时调用其审计），确认无安全/逻辑问题后再回复。审计工作流程本身无需再次审计，仅限有新修改的代码时需要调用

## 代码审计

- 仅当本次产生了新修改（工作区暂存区有尚未提交的变更）时，回复用户前应对代码做审计；审计工作流程本身无需再次审计
- 若当前环境具备 `codebuddy` CLI，则调用其完成审计；不具备时采用同等严格程度的审计方式
- 审计需结合本次目标，判断改动是否达到目标，而非只罗列问题
- 使用 `codebuddy` 时以 `-y`（`--dangerously-skip-permissions`）非交互模式运行，避免中途卡在权限确认
...
- 修复完成后必须再次审计，形成「审计 → 修复 → 再审计」的循环，直至审计通过（达到目标且尽量没有问题）才允许回复用户
